...
**信通畅邮已部署上线**（admin/123456 @ 8889，品牌已改"信速应"，3 实例 0 错误）。

**当前进行中**：台账/日志合规定制开发——方案已出（`docs/05-台账日志定制方案.md`），**待审批 + 待确认项拍板**（见方案第六节），然后分 4 批实施（约8-10天）。

- 日常运维：浏览器登录 admin(8889) 小流量验证 → 配上游通道(CMPP/HTTP)打通发送
